...
exports.run = async(client, message, args) => {
 if (!message.member.roles.cache.has(ayarlar.vmutesorumlusu) && !message.member.hasPermission('ADMINISTRATOR')) return message.channel.send(new Discord.MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setColor(0x00bfff).addField("Yetersiz Yetki",`Bu Komutu Kullanmak içi Yeterli Yetkiniz Yok`)).then(m => m.delete({timeout: 7000}));
  ...
  await uye.roles.add(ayarlar.vmuted).catch();
  ...
  client.channels.cache.get(ayarlar.vmutelog).send(new Discord.MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setColor(0x00bfff).setDescription(`${uye} İsimli Kullanıcı Ses Kanallarında Susturuldu.\n\n• Yetkili: <@!${message.author.id}> \`${message.author.id}\` \n• Mutelenen Üye: <@!${uye.id}> \`${uye.id}\` \n\n• Sebep: **${reason}**`)).catch();
};
exports.config = {
    name: "vmute",
    guildOnly: true,
    aliases: ["Vmute","Vmuted","vmuted"]
}