...
if(etiketlenenKişi.roles.cache.has(ayarlar.vipRol)) return message.channel.send(Tuomainen.setDescription(`Kullanıcıdan başarıyla vip <@&${ayarlar.vipRol}> rolü alındı!`)).then(etiketlenenKişi.roles.remove(ayarlar.vipRol))

etiketlenenKişi.roles.add(ayarlar.vipRol)

message.react(client.emojis.cache.get(ayarlar.yes))

message.channel.send(Tuomainen.setDescription(`Kullanıcıya başarıyla <@&${ayarlar.vipRol}> rolü verildi!`))
